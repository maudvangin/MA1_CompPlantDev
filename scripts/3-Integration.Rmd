...
#AddMetaData to LJ_root, ploïdy uit LJ_root_ortho (Idents)
...
Visualize UMAP of Lotus orthologues 

```{r}
LJ_ortho_umap <- DimPlot(LJ_root_ortho, reduction = "umap", pt.size = 2) + labs(title = "", x = "UMAP1", y = "UMAP2") + theme(axis.title = element_text(size = 25), legend.text = element_text(size = 30), axis.line = element_line(size = 1), legend.key.size = unit(2.5, "lines"), axis.text = element_text(size = 20))
  
LJ_ortho_umap

pdf(file = "Integration_LJ_ortho_umap.pdf", width = 18, height = 10)
LJ_ortho_umap
dev.off()
```
...
celltype <- DimPlot(LJ_root, reduction = "umap60", pt.size = 2, group.by = "predicted.id") + labs(title = "", x = "UMAP1", y = "UMAP2") + theme(axis.title = element_text(size = 25), legend.text = element_text(size = 30), axis.line = element_line(size = 1), legend.key.size = unit(2.5, "lines"), axis.text = element_text(size = 20))
celltype

pdf(file = "Integration_LJ_UMAP_LabelTransfer.pdf", width = 18, height = 10)
celltype
dev.off()
```
...
compSC_umap <- DimPlot(compSC.combined, group.by = "orig.ident", pt.size = 1.5, ) + labs(title = "", x = "UMAP1", y = "UMAP2") + theme(axis.title = element_text(size = 25), legend.text = element_text(size = 30), axis.line = element_line(size = 1), legend.key.size = unit(2.5, "lines"), axis.text = element_text(size = 20))
compSC_umap

pdf(file = "Integration_combined_umap.pdf", width = 18, height = 10)
compSC_umap
dev.off()
```
...
compSC_umap_int <- DimPlot(compSC.combined, group.by = "orig.ident", pt.size = 1.5) + labs(title = "", x = "UMAP1", y = "UMAP2") + theme(axis.title = element_text(size = 25), legend.text = element_text(size = 30), axis.line = element_line(size = 1), legend.key.size = unit(2.5, "lines"), axis.text = element_text(size = 20)) 
compSC_umap_int

pdf(file = "Integration_combined_umap_integrated.pdf", width = 18, height = 10)
compSC_umap_int
dev.off()
```
...
```{r}
p1 <- DimPlot(AT_root, reduction = "umap", group.by = "celltype.id", dims = c(1,2), label = TRUE, label.size = 6, repel = TRUE, pt.size = 1.5) + NoLegend() + ggtitle("Reference annotations")

p2 <- DimPlot(LJ_root_ortho, reduction = "ref.umap", dims = c(1,2), group.by = "predicted.celltype", label = TRUE, label.size = 6, repel = TRUE, pt.size = 1.5) + NoLegend() + ggtitle("Query transferred labels")

LJ_root <- AddMetaData(object = LJ_root, metadata = LJ_root_ortho@meta.data) 
Idents(object = LJ_root) <- LJ_root_ortho@meta.data[["predicted.celltype"]] 

p3 <- DimPlot(LJ_root, reduction = "umap60", group.by = "predicted.celltype", label.size = 6, repel = TRUE, dims = c(1,2), pt.size = 1.5) + ggtitle("Query transferred labels on original UMAP")
...
```{r}
pdf(file = "AT_unimodalUMAP.pdf", width = 18, height = 10)
p1
dev.off()

pdf(file = "LJ_unimodalUMAP.pdf", width = 18, height = 10)
p2
dev.off()
```
...
Look for Orthologues on PLAZA vs BLAST
...
CyclinB1-1, cell division marker
```{r}
AT_CYCB1 <- FeaturePlot(AT_root, reduction = "umap60", features = 'AT4G37490', pt.size = 2) + labs(title = "CYCB1-1", x = "UMAP1", y = "UMAP2") + theme(axis.title = element_text(size = 25), legend.text = element_text(size = 30), axis.line = element_line(size = 1), legend.key.size = unit(2.5, "lines"), axis.text = element_text(size = 20))

LJ_CYCB1 <- FeaturePlot(LJ_root, reduction = "umap60", features = 'LotjaGi1g1v0659100', pt.size = 2) + labs(title = "CYCB1-1", x = "UMAP1", y = "UMAP2") + theme(axis.title = element_text(size = 25), legend.text = element_text(size = 30), axis.line = element_line(size = 1), legend.key.size = unit(2.5, "lines"), axis.text = element_text(size = 20))

AT_CYCB1 + LJ_CYCB1

pdf(file = "AT_CYCB1.pdf", width = 18, height = 10)
AT_CYCB1
dev.off()

pdf(file = "LJ_CYCB1.pdf", width = 18, height = 10)
LJ_CYCB1
dev.off()
```
...
```{r}
AT_AT5G05500 <- FeaturePlot(AT_root, reduction = "umap60", features= "AT5G05500", pt.size = 2) + labs(title = "AT5G05500", x = "UMAP1", y = "UMAP2") + theme(axis.title = element_text(size = 25), legend.text = element_text(size = 30), axis.line = element_line(size = 1), legend.key.size = unit(2.5, "lines"), axis.text = element_text(size = 20))

LJ_AT5G05500 <- FeaturePlot(LJ_root, reduction = "umap60", features= "AT5G05500", pt.size = 2) + labs(title = "AT5G05500", x = "UMAP1", y = "UMAP2") + theme(axis.title = element_text(size = 25), legend.text = element_text(size = 30), axis.line = element_line(size = 1), legend.key.size = unit(2.5, "lines"), axis.text = element_text(size = 20))

AT_AT5G05500 + LJ_AT5G05500

pdf(file = "AT_AT5G05500.pdf", width = 18, height = 10)
AT_AT5G05500
dev.off()

pdf(file = "LJ_AT5G05500.pdf", width = 18, height = 10)
LJ_AT5G05500
dev.off()
```


Heatmaps - Arabidopsis cluster annotation vs Lotus
...
# Divide each element of the matrix by the adjusted row sum and column sum
row_col_sums <- outer(row_sums, col_sums, "+")
row_col_sums_norm <- row_col_sums - cluster_overlap2
cluster_overlap_norm <- cluster_overlap2 / row_col_sums_norm * 100
```
...
```{r}
# Filter AT markers: p value < 0.05 and logfoldchange > 1 (higher expression)
AT_root_markersf <- filter(AT_root_markers, p_val_adj < 0.05 & avg_log2FC > 1)
```

```{r}
AT_LJ_markers <- inner_join(AT_root_markersf, LJ_root_markers, by = 'gene' ) %>% select(c('gene','cluster.x','cluster.y'))
...
pheatmap(cluster_overlap_norm, display_numbers = cluster_overlap)
```
