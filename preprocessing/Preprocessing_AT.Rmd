...
DimPlot(AT_root, reduction = "umap10", cols = "black")
```

```{r}
AT_root <- RunUMAP(AT_root, dims = 1:30, reduction.name = "umap30")
DimPlot(AT_root, reduction = "umap30", cols = "black")
```

```{r}
AT_root <- RunUMAP(AT_root, dims = 1:60, reduction.name = "umap60")
#AT_root <- Idents(object = AT_root, id = AT_root@meta.data[["seurat_clusters"]])
...
```{r}
AT_root <- RunUMAP(AT_root, dims = NULL, features = VariableFeatures(object = AT_root), reduction.name = "umap_all")
DimPlot(AT_root, reduction = "umap_all", cols = "black")
```
...
ploidyplot <- DimPlot(AT_root, group.by="ploidy.ID.P", cols=color, reduction = 'umap60', dims = c(1,2)) + labs(x = "", y = "", title = "Ploidy levels")

#ploidyplot13 <- DimPlot(AT_root, group.by="ploidy.ID.P", cols=color, reduction = 'umap60', dims = c(1,3)) + labs(x = "", y = "", title = "Ploidy levels")

#ploidyplot23 <- DimPlot(AT_root, group.by="ploidy.ID.P", cols=color, reduction = 'umap60', dims = c(2,3)) + labs(x = "", y = "", title = "Ploidy levels")

#pdf(file = "AT_ploidyplot.pdf", width = 16, height = 16)
ploidyplot
#dev.off()
#ploidyplot13
#ploidyplot23
...
```{r}
#pdf(file = "AT_timezoneplot.pdf", width = 16, height = 16)
timezone
#ploidyplot
#dev.off()
```
...
FeaturePlot(object = AT_root, features = 'AT4G37490', reduction = 'umap60')
```
...
```{r}
new_cluster_ids <- c("Lateral Root Cap", "Meristematic cell", "Columella (Root Cap)", "Trichoblast", "Meristematic cell", "Stele", "Atrichoblast", "Epidermis", "Endodermis", "Cortex", "Stele", "Root cap", "Root cap","Root cap")
names(new_cluster_ids) <- levels(AT_root)
AT_root <- RenameIdents(AT_root, new_cluster_ids)
DimPlot(AT_root, reduction = "umap60", label = TRUE, pt.size = 0.5) + NoLegend()
```
...
```{r}
#rm(celltype, counts_wt, endo_exp, Long, ploidy, ploidy_cor, ploidy_pvalue, ploidy_stat, ploidyplot, plot1, plot2, processed_counts, Rad, time, time_cor, time_pvalue, time_stat, timezone, color, geneNames_wt, new_cluster_ids, order, palette, ploidy_ident, ploidy_label, ploidy_max, ploidy_maxp, time_ident, time_label, time_max, time_maxp, top10_AT)
